...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        log_std_list = torch.stack(( self.scale(log_std_1), self.scale(log_std_2), self.scale(log_std_3), self.scale(log_std_4)), 1)
        mean_list = torch.stack((mean_1, mean_2, mean_3, mean_4), 1)
...
class SAC(object):
    def __init__(self, state_dim, action_dim, max_action, args, K_primitives=4):
        self.training = True
        self.actor = Actor(state_dim, action_dim).to(device)

        # separate gating network:
        self.gating_func = gating_func(state_dim, hidden_dim=256, K_primitives=4).to(device)
        ...
        if args.use_lr:
            ...
            self.gating_optimizer = torch.optim.Adam(self.gating_func.parameters(), lr=args.policy_lr)
        else:
